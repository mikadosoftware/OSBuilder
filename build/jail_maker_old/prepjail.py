...
def get_ifconfig_prefix():
    """the ifconfig prefix we are looking for is basically ifconfig_bge0 or ifconfig_rl0
    however it is hard to tell which interface / driver we want - I need to write something that parses 
    the existing rc.conf and works it out. """

#    ifconfig = "ifconfig_bge0"

    return 'ifconfig_msk0'
...
def each_time_host_prep(**details):
    ...
    ifconfig_prefix = get_ifconfig_prefix()
    details['ifconfig_ct'] = ifconfig_ct
    ifconfig_alias = ifconfig_prefix + '_alias%(ifconfig_ct)s="inet %(ip)s netmask 255.255.255.255"' % details   
    ifconfig_alias_direct = ifconfig_prefix.replace("_"," ") + " inet %(ip)s netmask 255.255.255.255 alias" % details     
...
if __name__ == '__main__':
    ...
    one_time_host_prep()    
    ifconfig_alias = each_time_host_prep(jailname=hostname, ip=ip, domain=DOMAIN)
    configjail(jailname=hostname, ip=ip, domain=DOMAIN, if_alias=ifconfig_alias)
