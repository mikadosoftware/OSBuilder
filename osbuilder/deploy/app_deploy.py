...
class AppDeploy(object):
    ...
    def logInstall(self, app, rev):
        """The default logger. Do not override this. Instead, use the install_log hook to 
        pass a callable
        """
        log_url = "http://compswww.office.pirc.co.uk/cgi-bin/admin/store_deployment.py"
        log_data = {'app': app, 'rev': rev, 'deploy_file':self.deploy_file, 'options': str(self.options)}
        ...
        top_level_url = "http://compswww.office.pirc.co.uk/cgi-bin/admin"
        password_mgr.add_password(None, top_level_url, "pircuser", "pircpass")
